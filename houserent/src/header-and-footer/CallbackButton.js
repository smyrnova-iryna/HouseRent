...
// );
// }
export default function CallbackButton ({data, handleToggle}) {
  return (
      // <div className='Callback-button-container Pulse-Icon'>
        <button onClick={handleToggle} className='Callback-Button'>
        {/* <img alt="Icon" className={`Header-Callback-Icon`} src={require(`../img/${iconUrl}`)}>
        </img> */}
          <PhoneCallbackIcon className='Header-icon' />
          {/* <img alt="Icon" src={require(`../img/${iconUrl}`)}></img> */}
        <span >{data.header.callback.text}</span>
        </button>
      // </div>
  );
}
